				...
				try {
					...
					for(i=0;i<prodotti.size();i++) {
						if(prodotti.get(i).getPrezzo() <= prezzomax && prodotti.get(i).getPrezzo() >= prezzomin) {
							modello.addRow(new Object[]{prodotti.get(i).getCodP(), prodotti.get(i).getNome(), prodotti.get(i).getDescrizione(), "€"+prodotti.get(i).getPrezzo() ,prodotti.get(i).getTipo(), "-", 0, "+"});
						}
					}
		...
		btn_conferma.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (controller.isSpesaVuota()) {
					JOptionPane.showMessageDialog(contentPane,
						    "Inserisci prodotti nel carrello",
						    "Errore nell'ordine",
						    JOptionPane.ERROR_MESSAGE);
				}else {
