	...
	public WinConferma(ControllerCliente controllerCliente) {
		...
		btn_conferma.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					c.setCodC("nextval('seq_codc')");
					c.setCodA(attivita.get(cb_attivita.getSelectedIndex()).getCodA());
					c.setCivico(txt_civico.getText());
					c.setVia(txt_via.getText());
					c.setPrezzo((float)0.0);//il prezzo totale è gestito dal database
					c.setCodCarta(txt_codcarta.getText());
					c.setDataO("current_timestamp");
					c.setNote(txt_note.getText());
					if(rb_contanti.isSelected()) {
						c.setMetodoP("contanti");
